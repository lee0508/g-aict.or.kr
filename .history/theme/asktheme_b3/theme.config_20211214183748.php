...
// $theme_config['header'] = "header_fulldown.php";
...
if (!function_exists('youtube_link_upload')) {
    ...
    function youtube_link_upload($wr_link)
    {
        ...
        if (strstr($wr_link, 'youtube.com') || strstr($wr_link, 'youtu.be')) {
            ...
            parse_str(parse_url($wr_link, PHP_URL_QUERY), $my_arr);
            $wr_file_name = "youtube_{$wr_id}" . $bo_table . "_" . md5($youtube_url) . '.jpg';
            ...
            if (!file_exists($wr_file_path)) {
                ...
                file_put_contents($wr_file_path, file_get_contents_curl("https://img.youtube.com/vi/{$youtube_url}/sddefault.jpg")); // 613 line에 함수 추가됨
                //이미지 정보
...
//style selector
if (!function_exists('style_selector')) {
    ...
    {
        global $is_admin, $g5, $board, $menu_type;
        if (stristr($_SERVER['PHP_SELF'], 'preview')) {
            return false;
        }

        //데모서버에서만 동작
        if ($_SERVER['SERVER_ADDR'] == '172.16.0.4') {
            include_once G5_THEME_PATH . DIRECTORY_SEPARATOR . 'selector' . DIRECTORY_SEPARATOR . 'theme_style.php';
        } else {
            return false;
        }
    }
...
if (!function_exists('ask_get_pretty_url')) {
    /**
     * 내용관리 짧은주소
     *
     * @param [type] $folder
     ...
     * @return void
     */
    function ask_get_pretty_url($folder, $no = '', $query_string = '', $action = '')
    {
        //그누 5.4 이상
...
if (!function_exists('ask_get_pretty_board_url')) {
     ...
     * @return void
     */
    function ask_get_pretty_board_url($bo_table, $no = '', $query_string = '', $action = '')
    {
        //그누 5.4 이상
        if (defined('THEME_SHORT_URL') && THEME_SHORT_URL == true) {
...
if (!function_exists('ask_short_url_clean')) {
     ...
     * @return void
     */
    function ask_short_url_clean($url)
    {
...
$theme_config = array_merge($theme_config, $shop_theme_config);
*/
